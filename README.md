...
│   └── backend-repo/
├── packages/
│   └── user.ts (common shared logic)
├── package.json
└── turbo.json
```

The monorepo setup uses `turbo` to manage both the frontend and backend projects in a single repository, with the `user.ts` file as the shared logic.
